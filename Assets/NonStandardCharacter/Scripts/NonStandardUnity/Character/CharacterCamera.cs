...
namespace NonStandard.Character {
	public class CharacterCamera : MonoBehaviour {
		...
		[Tooltip("which camera will do the following. if null, will grab this object's camera")]
		...
		public Camera Camera => _camera != null ? _camera : _camera = RefreshCamera();
		...
		public bool RecalculateDistance() {
			...
			if (target != null) {
				Vector3 delta = camTransform.position - target.position;
		...
		public bool RecalculateRotation() {
			...
			Vector3 right = Vector3.Cross(camTransform.forward, Vector3.up);
			if(right == Vector3.zero) { right = -camTransform.right; }
			Vector3 straightForward = Vector3.Cross(Vector3.up, right).normalized;
			pitch = Vector3.SignedAngle(straightForward, camTransform.forward, -right);
		...
		public void Update() {
			...
			if (zoom != 0) {
				...
				if (targetDistance > maxDistance) { targetDistance = maxDistance; }
				if (target == null) {
					camTransform.position += camTransform.forward * zoom;
				}
				OrthographicCameraDistanceChangeLogic();
			}
			if (rotH != 0 || rotV != 0) {
			...
			if (target != null) {
				...
				if (clipAgainstWalls && Physics.Raycast(target.position, -camTransform.forward, out hitInfo, targetDistance)) {
		...
		public void LateUpdate() {
			...
			if(target != null) {
				camTransform.position = target.position - (camTransform.rotation * Vector3.forward) * distanceBecauseOfObstacle;
			}
		}
	}
}
